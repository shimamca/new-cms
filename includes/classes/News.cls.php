	...
	function updateNews($request) {
		...
		if( isset($_FILES['file_name']['name']) && $_FILES['file_name']['name']!="" ) {
			...
			if($file_name) {
				...
				$sql = "UPDATE ".NEWS_EVENTS." SET  title='$title', publish_date='$publish_date',  file_name='$file_name', description='$description', category='$category' where id='$id'";
			} else {
				$sql = "UPDATE ".NEWS_EVENTS." SET  title='$title', publish_date='$publish_date', description='$description', category='$category' where id='$id'";
			}
			$res = $this->update($sql);
