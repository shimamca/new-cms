...
class PhotoGalleries extends SiteData {
	...
	function getPhotosLimit() {
		//$sql = "SELECT a.*,b. from ".PHOTO_GALLERYS." where photo_status= '1' order by photo_id desc LIMIT 0,9";
		
		$sql = "SELECT a.*, b.category_url from ".PHOTO_GALLERYS." a LEFT OUTER JOIN ".PHOTO_CATEGORYS." b ON b.category_id=a.photo_category where  a.photo_status='1' AND b.category_status='1' order by a.photo_id desc LIMIT 0,9";
		$res = $this->getData($sql);
		return $res;
	}
